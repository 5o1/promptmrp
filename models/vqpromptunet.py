...
from .modules.context import register_extra_loss
...
class TimeDepthFuseBlock(nn.Module):
    ...
    def forward(self, x: torch.Tensor):
        ...
        x : b t s c h w
        """
        tfused = self.tatt(x, x)
        dfused = self.datt(x, x)
        res = self.decoder(tfused + dfused)
        res = res + x
        return res
...
class VQPromptUnet(nn.Module):
    def __init__(self,
        ...
        # Decoder - 3 UpBlocks
        self.prompt_level3 = VQPromptBlock(feature_dim[2], prompt_dim[2], int(feature_dim[2] * 1.3),len_prompt[2], 1, 1, reduction, prompt_size[2],0.99,kernel_size,bias, act).rearrange("b t s c h w", "$ c h w", for_out = [0])
        ...
        self.prompt_level2 = VQPromptBlock(feature_dim[1], prompt_dim[1], int(feature_dim[1] * 1.3),len_prompt[2], 1, 1, reduction, prompt_size[1],0.99,kernel_size,bias, act).rearrange("b t s c h w", "$ c h w", for_out = [0])
        ...
        self.prompt_level1 = VQPromptBlock(feature_dim[0], prompt_dim[0], int(feature_dim[0] * 1.3),len_prompt[2], 1, 1, reduction, prompt_size[0],0.99,kernel_size,bias, act).rearrange("b t s c h w", "$ c h w", for_out = [0])
